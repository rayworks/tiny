...
void doit(int fd)
{
    ...
    if(strcasecmp(method, "POST") == 0) {
        int body_len = read_requesthdrs(&rio, 1);
    ...
    else if (strcasecmp(method, "GET")) {        //line:netp:doit:beginrequesterr
        clienterror(fd, method, "501", "Not Implemented",
                    "Tiny does not implement this method");
        return;
    } else{                                                   //line:netp:doit:endrequesterr
        // GET
        read_requesthdrs(&rio, 0);                              //line:netp:doit:readrequesthdrs
    }
    
    /* Parse URI from GET / POST request */
    ...
    if (is_static) { /* Serve static content */
        ...
        serve_static(fd, filename, sbuf.st_size, buf);        //line:netp:doit:servestatic
    }
...
void serve_static(int fd, char *filename, int filesize, char* conninfo)
{
    ...
    sprintf(buf, "%sFile size: %d \r\n", buf, filesize);
    printf("%s", buf);
    
    /* append the connection info */
    //Rio_writen(fd, conninfo, strlen(conninfo));
    
    /* Send response body to client */
    srcfd = Open(filename, O_RDONLY, 0);    //line:netp:servestatic:open
    
    char* pFile = (char*)malloc(filesize);
    memset(pFile, 0, filesize);
    
    //Rio_writen(srcfd, pFile, filesize);
    Rio_readn(srcfd, pFile, filesize);
    
    //srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0);//line:netp:servestatic:mmap
    //Close(srcfd);                           //line:netp:servestatic:close
    
    Rio_writen(fd, pFile, filesize);         //line:netp:servestatic:write
